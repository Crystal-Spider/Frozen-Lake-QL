class Agent
{
  ...
  private double getMaxReward(int state)
  {
    ...
    if(state >= 0 && state < states) //If the state is inside the game map (valid indices for the Q table).
    {
      max = Q[state][0];
  ...
  {
    return ((x-cellDim/2)+(y-cellDim/2)*cellNum)/cellDim;
  }
