class Lake
{
  ...
  public Lake()
  {
    ...
    cells.get(states-1).c = color(0,120,0);
    cells.get(states-1).kind = 3;
    ...
    */
    ...
    for(int c = 0; c < cellNum; c++)
    {
      holesPos.add((int)random(2, states-2));
    }
    for(int c = 2; c < states-2; c++)
    {
      if(c != cellNum && c != states-cellNum-1 && holesPos.contains(c))
      {
        cells.get(c).c = hole;
        cells.get(c).kind = 0;
      }
